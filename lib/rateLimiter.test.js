...
test('rateLimiter - purgeStaleEntries cleans expired records and keeps valid records', () => {
  resetRateLimits();
  checkRateLimit('client-purge-keep', 5, 100000);
  checkRateLimit('client-purge-delete', 5, 100000);
  purgeStaleEntries(100000, true);
  const remainingKeep = checkRateLimit('client-purge-keep', 5, 100000);
  assert.equal(remainingKeep.remaining, 3);

  // Expire all entries
  purgeStaleEntries(-1000, true);
  const fresh = checkRateLimit('client-purge-delete', 5, 100000);
